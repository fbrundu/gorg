...
/*** Insert data in a BST (recursive function) COMPARES THE FIELD "date" ************/
static nodo_t
tree_insert(nodo_t rp, dato_t d, int age) {
   ...
   if (data_compare(d, rp->val, 0) == 0) { /* BST allows ONLY one node for the specified data value */
      printf("Date already used. Try another..\n");
      return rp;
   }
   if (data_compare(d, rp->val, 0) < 0)
      rp->left = tree_insert(rp->left, d, age);
   else rp->right = tree_insert(rp->right, d, age);
   return rp;
}

/*** Estract data from a BST (recursive function) COMPARES THE FIELD "string" ************/
static nodo_t
tree_remove(nodo_t rp, dato_t din, dato_t *dout) {
   ...
   if (data_compare(din, rp->val, 0) < 0) {
      /* delete left */
